...
export default function Features() {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      raf = requestAnimationFrame(() => {
        ...
        section.style.setProperty("--feature-progress", raw.toString());
        const nextIndex = Math.min(N - 1, Math.floor(raw * N));
  ...
  return (
    <section id="features" ref={sectionRef} className="relative bg-white" style={{ height: `${N * 86}vh` }}>
      {/* Desktop / tablet: pinned sliding stage */}
      <div className="feature-stage hidden lg:flex sticky top-0 h-screen flex-col overflow-hidden">
        <div className="mx-auto max-w-7xl px-6 pt-24 w-full flex-none">
          ...
          <h2 className="text-[30px] xl:text-[36px] font-black tracking-tight text-[#15213b] mb-2">
            One intelligence fabric for every Salesforce org
          </h2>
          <p className="text-[15px] text-[#60708d] leading-[1.6] max-w-xl">
            From executive health scores to grounded AI advisory — continuous, evidence-based, and read-only.
          </p>
        </div>

        <div className="mx-auto max-w-7xl px-6 w-full flex-1 grid grid-cols-[244px_1fr] gap-10 items-center min-h-0 mt-6">
          {/* Left rail: numbered nav 01 → 08 with progress spine */}
          <div className="feature-rail relative h-full max-h-[456px] self-center p-4">
            <div className="absolute left-8 top-5 bottom-5 w-px bg-[#dce6f7]">
              <div
                className="feature-progress-fill absolute top-0 left-0 w-full bg-gradient-to-b from-[#1552D2] to-[#7c5ce0]"
              />
            </div>
            <div className="space-y-1 pl-11">
              {FEATURES.map((f, i) => (
                <div
                  key={f.num}
                  className={`relative flex items-center gap-3 -ml-3 rounded-xl px-3 py-2 transition-all duration-300 ${
                    i === activeIndex ? "bg-[#f0f5ff] shadow-[0_8px_22px_rgba(50,83,140,0.08)]" : "hover:bg-white/70"
                  }`}
                >
                  <div
                    className={`absolute -left-7 w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-black transition-all duration-300 ${
                      i <= activeIndex
                  ...
                  <div
                    className={`text-[12px] font-bold tracking-tight transition-all duration-300 ${
                      i === activeIndex ? "text-[#15213b]" : i < activeIndex ? "text-[#60708d]" : "text-[#91a0b8]"
                    }`}
                  >
                    {f.tag}
                  </div>
          ...
          <div className="relative h-full min-h-0 max-h-[500px] flex items-center">
            {FEATURES.map((feat, i) => {
              ...
              const style = active
                ? { opacity: 1, transform: "translate3d(0, 0, 0) scale(1)", zIndex: 10, pointerEvents: "auto" }
                : {
                  opacity: 0,
                  transform: `translate3d(${passed ? -8 : 8}%, ${passed ? -12 : 12}px, 0) scale(0.985)`,
                  zIndex: 1,
                  pointerEvents: "none",
                };
              return (
                <div
                  key={feat.num}
                  data-index={i}
                  className="feat-stage-card absolute inset-0"
                  style={style}
                >
                  <div className="feature-card h-full border rounded-3xl p-7 xl:p-9 overflow-hidden">
                    <div className="grid grid-cols-2 gap-8 h-full items-center">
                      <div className="min-w-0">
                        <div className="text-[11px] font-black tracking-[0.12em] text-[#1552D2] mb-2">{feat.num} · {feat.tag}</div>
                        <h3 className="text-[21px] xl:text-[24px] font-black tracking-tight text-[#15213b] mb-3 leading-tight">{feat.title}</h3>
                        <p className="text-[14px] xl:text-[15px] text-[#52627f] leading-[1.6] mb-4 line-clamp-4">{feat.body}</p>
                        <ul className="space-y-2">
                          {feat.bullets.map(b => (
                            <li key={b} className="flex items-start gap-2.5 text-[13px] xl:text-[14px] text-[#354561] font-medium">
                              <Check />{b}
                            </li>
                          ))}
                        </ul>
                      ...
                      <div className="feature-viz rounded-2xl p-5 h-full max-h-[360px] overflow-hidden flex flex-col">
        ...
        <div className="mx-auto max-w-7xl px-6 w-full flex-none py-6 flex items-center justify-between">
          <span className="text-[11px] font-mono tracking-wide text-[#60708d]">
            ...
            {FEATURES.map((f, i) => (
              <span
                ...
                className={`h-1 rounded-full transition-all duration-300 ${
                  i === activeIndex ? "w-8 bg-[#1552D2]" : i < activeIndex ? "w-2 bg-[#9fc1ff]" : "w-1.5 bg-[#dce6f7]"
                }`}
              />
            ))}
          </div>
      ...
      <div className="lg:hidden py-20 bg-white">
        <div className="mx-auto max-w-2xl px-6">
          <div className="text-center mb-14 reveal">
            ...
            <h2 className="text-[30px] font-black tracking-tight text-[#15213b] mb-4">
              One intelligence fabric for every Salesforce org
            </h2>
